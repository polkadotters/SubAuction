...
pub mod pallet {
	...
	pub struct GenesisConfig<T: Config> {
		pub tokens: Vec<GenesisTokens<T::AccountId, <T as orml_nft::Config>::ClassData, <T as orml_nft::Config>::TokenData>>
	}
	...
	pub fn toggle_lock(account: &T::AccountId, token_id: (T::ClassId, T::TokenId)) -> DispatchResultWithPostInfo {
		...
		orml_nft::Tokens::<T>::mutate_exists(token_id.0, token_id.1, |token| -> DispatchResultWithPostInfo {
			if let Some(ref mut token) = token {
				...
				// fix clone
				Self::deposit_event(Event::NFTTokenMintedLockToggled(account.clone(), token_id.0, token_id.1, token.data.locked));
			}
			Ok(().into())
		})?;
		Ok(().into())
